...
const OCardImageWrapper = styled.div`
    ...
    -webkit-transition: border-width 0.7s ease-in-out;
            transition: border-width 0.7s ease-in-out;
    border-radius: calc(${(p) => p.theme.size.innerPadding} + ${(p) => p.theme.size.borderRadius});
    animation: ${(p) => (p.inView) ? radiusChange : radiusChangeInverse} 0.7s ease-in-out;
...
const OCardContent = styled.div`
    width: ${(p) => (p.inView) ? '74rem' : '67.8rem'};;
    min-height: 75vh;
    position: relative;
    margin-top: -40vh;
    margin-bottom: 20vh;
    margin-left: ${(p) => p.theme.size.innerPadding};
    padding-left: ${(p) => (p.inView) ? 0 : '6.2rem'};
    -webkit-transition: padding-left 0.7s ease-in-out;
